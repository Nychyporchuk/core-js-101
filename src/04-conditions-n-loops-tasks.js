...
function findFirstSingleChar(str) {
  const charCounts = Array.from(str).reduce((acc, char) => {
    acc[char] = (acc[char] || 0) + 1;
    return acc;
  }, {});
  return Array.from(str).find((char) => charCounts[char] === 1) || null;
}
...
function reverseInteger(num) {
  ...
  return num < 0 ? -parseInt(reversedStr, 10) : parseInt(reversedStr, 10);
}
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    throw new Error('Base must be between 2 and 10 inclusive');
  }
